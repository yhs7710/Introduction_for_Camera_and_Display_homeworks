...
def main():
    ...
    # read images
    earlier_frm_rgb = cv2.imread('/database/IVS/동해과제/other-color-twoframes/other-data/Beanbags/frame10.png')
    real_frm_rgb = cv2.imread('/database/IVS/동해과제/other-gt-interp/other-gt-interp/Beanbags/frame10i11.png')
    later_frm_rgb = cv2.imread('/database/IVS/동해과제/other-color-twoframes/other-data/Beanbags/frame11.png')
    ...
    # open motion flow
    with open('/database/IVS/동해과제/other-color-twoframes/other-data/Beanbags/Beanbags.flo', 'r') as flo:
    ...
    # setting limit on motion vector not to go out from the picture
    for (x,y) in itertools.product(range(0, motion_field.shape[0]), range(0, motion_field.shape[1])):
        motion_field[x,y,0] = max(motion_field[x,y,0]+x, 0)-x
        motion_field[x,y,1] = max(motion_field[x,y,1]+y, 0)-y
        motion_field[x,y,0] = min(motion_field[x,y,0]+x+1, motion_field.shape[0])-x-1
        motion_field[x,y,1] = min(motion_field[x,y,1]+y+1, motion_field.shape[1])-y-1
        motion_field[x,y,0] = -(max(-motion_field[x,y,0]+x, 0)-x)
        motion_field[x,y,1] = -(max(-motion_field[x,y,1]+y, 0)-y)
        motion_field[x,y,0] = -(min(-motion_field[x,y,0]+x+1, motion_field.shape[0])-x-1)
        motion_field[x,y,1] = -(min(-motion_field[x,y,1]+y+1, motion_field.shape[1])-y-1)
    ...
    # # remove padded area
    # predicted_frm_FOBMC = predicted_frm_pad_FOBMC[2*expand_pixels:-2*expand_pixels, 2*expand_pixels:-2*expand_pixels]
    
    # # store predicted frame
    # cv2.imwrite('3_predicted_frame_FOBMC.png',predicted_frm_FOBMC)
    
    # # store error image
    # error_image_FOBMC = abs(np.array(predicted_frm_FOBMC, dtype=float) - np.array(real_frm_gray[expand_pixels:-expand_pixels, expand_pixels:-expand_pixels], dtype=float))
    ...
    plt.quiver(motion_field_x, motion_field_y, scale=1, units='xy', width=0.1)
    ...
    def run(self, motion_field, earlier_frame, later_frame):
        ...
        for (blk_row, blk_col) in itertools.product(range(0, height-block_size+1, block_size), range(0, width-block_size+1, block_size)):
            ...
            if (up_l_candidate_blk_earlier[0]<0) or (up_l_candidate_blk_earlier[1]<0) or \
                ...
                print("up_l_candidate_blk_earlier[0] : ", up_l_candidate_blk_earlier[0], "low_r_candidate_blk_earlier[0] : ", low_r_candidate_blk_earlier[0])
                print("up_l_candidate_blk_earlier[1] : ", up_l_candidate_blk_earlier[1], "low_r_candidate_blk_earlier[1] : ", low_r_candidate_blk_earlier[1])
                print("up_l_candidate_blk_later[0] : ", up_l_candidate_blk_later[0], "low_r_candidate_blk_later[0] : ", low_r_candidate_blk_later[0])
                print("up_l_candidate_blk_later[1] : ", up_l_candidate_blk_later[1], "low_r_candidate_blk_later[1] : ", low_r_candidate_blk_later[1])
                continue
    ...
    def run(self, motion_field, earlier_frame, later_frame):
        ...
        for (blk_row, blk_col) in itertools.product(range(0, height-block_size+1, block_size), range(0, width-block_size+1, block_size)):
            ...
            for (pix_row, pix_col) in itertools.product(range(0,int(block_size)), range(0,int(block_size))):
                ...
                predicted_frame[blk_row+2*block_size-pix_row-1, blk_col+block_size+2*expand_pixels-pix_col-1, 0] += earlier_frame_blk[2*block_size-pix_row-1, 2*block_size-pix_col-1, 0]*(pix_row+pix_col)/(8*block_size-8)+later_frame_blk[2*block_size-pix_row-1, 2*block_size-pix_col-1, 0]*(pix_row+pix_col)/(8*block_size-8)
                predicted_frame[blk_row+2*block_size-pix_row-1, blk_col+block_size+2*expand_pixels-pix_col-1, 1] += earlier_frame_blk[2*block_size-pix_row-1, 2*block_size-pix_col-1, 1]*(pix_row+pix_col)/(8*block_size-8)+later_frame_blk[2*block_size-pix_row-1, 2*block_size-pix_col-1, 1]*(pix_row+pix_col)/(8*block_size-8)
                predicted_frame[blk_row+2*block_size-pix_row-1, blk_col+block_size+2*expand_pixels-pix_col-1, 2] += earlier_frame_blk[2*block_size-pix_row-1, 2*block_size-pix_col-1, 2]*(pix_row+pix_col)/(8*block_size-8)+later_frame_blk[2*block_size-pix_row-1, 2*block_size-pix_col-1, 2]*(pix_row+pix_col)/(8*block_size-8)
